...
#
# Package-set-wide opt-out of Haddock documentation. OPT-IN: nothing composes
# it unless a caller passes `disableHaddock = true` to `mkChannelExtension` or
# `mkHaskellOverlay`, or composes this extension directly. The default stays
# false for the same reason it does on `disableProfiling`: this changes how a
# consumer's packages are built, not just which versions they resolve, and that
...
# dependency documentation, and any Hoogle index over this set. That is the
# reason this stays opt-in rather than becoming the default.
_hself: hsuper: {
